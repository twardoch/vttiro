...
def distribute_words_over_duration(
    words: list[str], start_time: float, end_time: float, base_confidence: float = 0.95
) -> list[dict[str, Any]]:
    ...
    for i, word in enumerate(words):
        ...
        if clean_word:  # Only add non-empty words
            word_timestamps.append({"word": word, "start": word_start, "end": word_end, "confidence": base_confidence})
