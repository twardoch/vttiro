...
class AudioProcessor:
    ...
    def process_media_file(self, input_path: Path, output_path: Optional[Path] = None) -> Tuple[List[Path], Path]:
    ...
    def _create_working_directory(self, input_path: Path, output_path: Optional[Path]) -> Path:
        """Create working directory based on input/output paths."""
        if output_path:
            # Use output file location and basename
            work_dir = output_path.parent / f".vttiro_{output_path.stem}"
        else:
            # Use input file location and basename  
            work_dir = input_path.parent / f".vttiro_{input_path.stem}"
    ...
    def _chunk_audio_if_needed(self, audio_path: Path, work_dir: Path) -> List[Path]:
        ...
        # Check if chunking is needed
        if file_size_mb <= self.MAX_FILE_SIZE_MB and duration_minutes <= self.MAX_DURATION_MINUTES:
    ...
    def _get_audio_duration(self, audio_path: Path) -> float:
        ...
        try:
            ...
            logger.warning(f"Could not get audio duration, using file size estimate: {e}")
            # Fallback: estimate based on file size (rough approximation)
            file_size_mb = audio_path.stat().st_size / (1024 * 1024)
            return file_size_mb * 60  # Very rough estimate: 1MB ≈ 1 minute
    
    def _split_audio(self, audio_path: Path, work_dir: Path, chunk_duration: float) -> List[Path]:
        ...
        for i, end_time in enumerate(split_points + [total_duration]):
            ...
            if not chunk_path.exists():
                self._extract_audio_segment(audio_path, chunk_path, start_time, end_time)
    ...
    def _extract_audio_segment(self, input_path: Path, output_path: Path, 
        ...
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, check=True)
            logger.debug(f"Audio segment extracted: {output_path} ({start_time:.1f}s-{end_time:.1f}s)")
        except subprocess.CalledProcessError as e:
            logger.error(f"Audio segment extraction failed: {e.stderr}")
            raise RuntimeError(f"Failed to extract segment {start_time}-{end_time}: {e.stderr}")
