...
class VttiroConfigFactory(factory.Factory):
    ...
    transcription = factory.SubFactory(factory.DictFactory, {
    ...
    # Processing settings
    processing = factory.SubFactory(factory.DictFactory, {
    ...
    # Output settings
    output = factory.SubFactory(factory.DictFactory, {
...
class ErrorScenarioFactory(factory.Factory):
    ...
    error_type = factory.fuzzy.FuzzyChoice([
        'network', 'api', 'auth', 'rate_limit', 'processing', 'validation'
    ])
...
class PerformanceMetricsFactory(factory.Factory):
    ...
    operation = factory.fuzzy.FuzzyChoice([
        'transcription', 'video_processing', 'audio_segmentation', 'api_call'
    ])
...
class BatchProcessingScenarioFactory(factory.Factory):
    ...
    sources = factory.LazyAttribute(
        lambda obj: [f"source_{i}.mp4" for i in range(obj.source_count)]
    )
...
def create_multilingual_test_batch() -> List[TranscriptionResult]:
    ...
    return [
        MultilingualTranscriptionFactory(language=lang)
